...
int pfs_getattr (const char* path, struct stat* statbuf) {
	...
	char* realpath = g_hash_table_lookup (data->filetable, path);
	if (!realpath)
		return -ENOENT;
...
int pfs_unlink (const char* path) {
	...
	if (!realpath)
		return -ENOENT;
	g_hash_table_remove (data->filetable, path);
	return 0;
}
...
int pfs_rename (const char* path, const char* newpath) {
	...
	char* realpath = NULL;
	char* key = NULL;
	if (!g_hash_table_lookup_extended (data->filetable, path, (void**)&key, (void**)&realpath))
		return -ENOENT;
	free (key);
	g_hash_table_steal (data->filetable, path);
	g_hash_table_insert (data->filetable, strdup(path), realpath);
	return 0;
}
...
// int pfs_statfs (const char* path, struct statvfs *);
