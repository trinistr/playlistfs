...
run_test "Outputs current version with --version" sh -c "'$BIN' --version | grep -Eq 'PlaylistFS [0-9]+\.[0-9]+\.[0-9]+'"

run_test "-V run" "$BIN" -V
run_test "Outputs current version with -V" sh -c "'$BIN' -V | grep -Eq 'PlaylistFS [0-9]+\.[0-9]+\.[0-9]+'"
